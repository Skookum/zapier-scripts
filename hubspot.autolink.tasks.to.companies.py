...
def get_timestamp_minus_1_hr():
    ...
    one_hr_ago = current_time - datetime.timedelta(hours=1)
    ...
    for a in range(0, len(recent_task_array['results'])):
        ...
        if recent_task_array['results'][a]['engagement']['type']=="TASK" and status!="COMPLETED" and not recent_task_array['results'][a]['associations']['companyIds'] and recent_task_array['results'][a]['associations']['dealIds']:
            ...
            if company_id != "DEAL_NOT_ASSOCIATED_COMPANY":
                ...
                records += str(recent_task_array['results'][a]['engagement']['id']) + " :: " +str(recent_task_array['results'][a]['metadata']['subject']) + " :: " + str(response) + " :: " + str(name_response) + "\n"
...
#records = fmt_company_details(get_recent_tasks(base_url,payload,get_timestamp_minus_1_hr()))
records = update_based_on_deal_id(get_recent_tasks(base_url,payload,get_timestamp_minus_1_hr()),deal_id)

if records == "":
    records = "No Updates"

output = {'records_updated': records}